-- =============================================================================
-- 0036_signup_bonus_on_verification.sql
-- Ensure signup bonus is only awarded AFTER email or phone verification
-- =============================================================================

-- 1. Add tracking column for signup bonus
ALTER TABLE public.users 
ADD COLUMN IF NOT EXISTS signup_bonus_awarded boolean DEFAULT false;

-- 2. Retroactively mark existing users who received a 50.00 credit as having received the bonus
UPDATE public.users 
SET signup_bonus_awarded = true 
WHERE id IN (
  SELECT user_id 
  FROM public.wallet_ledger 
  WHERE amount = 50.00 AND type = 'credit'
);

-- 3. Update resolve_customer_identity to conditionally award the bonus
...
BEGIN
  ...
  ELSE
    ...
    VALUES (
      ...
      p_phone_verified, 
      'customer', 
      0.00, -- Initialize to 0, trigger will update if they get the bonus
      0,
