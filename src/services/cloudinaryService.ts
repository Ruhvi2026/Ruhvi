...
const CLOUD_NAME = process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME || process.env.VITE_CLOUDINARY_CLOUD_NAME || 'tfelmupe';
const UPLOAD_PRESET = process.env.NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET || process.env.VITE_CLOUDINARY_UPLOAD_PRESET || 'ruhvi_products';
 ...
 */
export function getOptimizedImageUrl(sourceUrl: string, transformations: string = 'f_auto,q_auto'): string {
  if (!sourceUrl) return '';
  ...
  if (sourceUrl.includes('res.cloudinary.com') && !sourceUrl.includes('/image/fetch/')) {
     ...
     return sourceUrl.replace('/upload/', `/upload/${transformations},f_auto,q_auto/`);
  }
  ...
  const finalTransformations = transformations 
    ? (transformations.includes('f_auto') ? transformations : `${baseTransformations},${transformations}`) 
    : baseTransformations;
