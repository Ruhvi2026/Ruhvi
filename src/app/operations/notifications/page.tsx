...
const getIconForCategory = (category: string) => {
  switch (category) {
    ...
    case 'LOGISTICS':
      return { icon: Truck, color: 'text-orange-500', bg: 'bg-orange-500/10' };
    case 'INVENTORY':
...
const getLinkForNotification = (notif: Notification) => {
  if (notif.reference_type === 'quality_control_log')
    return '/operations/quality-control';
  if (notif.reference_type === 'shipment')
    return `/portal-orders/shipments?id=${notif.reference_id}`;
...
export default function OperationsNotifications() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('operations_notifications')
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
  ...
  const fetchNotifications = async () => {
    const { data } = await supabase
      .from('notifications')
      .select('*')
      .eq('target_department', 'operations')
      ...
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-500">
            Manage real-time alerts, logistics updates, and QC reports
          </p>
        </div>
